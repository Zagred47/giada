# Catalogo SQL — bozza da revisionare

Queste sono proposte di query versionate per consultazione, non nuove viste installate e non operazioni sul database. Le query non vengono eseguite automaticamente.

## Come leggere il catalogo

- **Esatta** significa fedele ai collegamenti registrati, non scientificamente vera.
- **Shortlist** significa casi candidati da valutare: non una conclusione automatica.
- **Lacuna documentale** significa assenza di registrazioni, non assenza di conoscenza nel mondo.
- Le query numeriche non producono p-value, indipendenza statistica o attribuzioni causali automatiche.
- Il mirror è vuoto: la verifica sul mirror copre sintassi/schema. I casi sintetici separati coprono alcuni comportamenti, non tutti i dati futuri.

## Parametri

I parametri con suffisso `_id` sono record ID Airtable, non nomi. `model_a/model_b`, livelli e componenti sono anch'essi ID.
`max_depth` deve essere un intero positivo ragionevole (es. 12); i cammini sono semplici e non espandono la ricorrenza nel tempo.
`axis` ammette solo i valori elencati in Q21.
`expected_strata_json` deve essere un array JSON di stringhe univoche.
`needle` deve essere una stringa non vuota.
Per Q17, i due fattori devono essere distinti, e i livelli 0/1 distinti e appartenenti al relativo fattore.

Il comando CLI `query` attuale non accetta binding. Per usare questi template senza interpolare valori nel testo SQL:
...
root = Path("research_memory")
catalog = json.loads((root / "queries/query_catalog_review.json").read_text(encoding="utf-8"))
query = next(q for q in catalog if q["id"] == "Q08")

with sqlite3.connect((root / "data/research_memory.sqlite").resolve().as_uri() + "?mode=ro", uri=True) as conn:
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA query_only=ON")
    rows = conn.execute(query["full_sql"], {"claim_id": "ID_REALE_DA_SELEZIONARE"}).fetchmany(200)
    for row in rows:
        print(dict(row))
```

Le CTE sono ripetute intenzionalmente: ciascuna query è indipendente e non richiede installare viste ausiliarie.
La CTE `one` accetta solo relazioni con un estremo; i link ambigui vanno prima esaminati con Q34.
La mancata obbligatorietà può causare esclusioni dai JOIN: non interpretare zero righe come dimostrazione di assenza.

## Indice

| ID | Domanda / lente | Natura |
|---|---|---|
| [Q01](#q01) | Scheda strutturale del componente | Esatta sui collegamenti registrati |
| [Q02](#q02) | Decomposizione ricorsiva del modello | Esatta, con limite di profondità |
| [Q03](#q03) | Ingressi, uscite e dipendenze dirette | Esatta per il grafo dichiarato |
| [Q04](#q04) | Percorsi di informazione tra due componenti | Topologica, non prova causale |
| [Q05](#q05) | Differenze tra componenti di due versioni | Esatta solo con identificatori locali stabili |
| [Q06](#q06) | Inventario matematico e connessioni per versione | Inventario per confronto umano |
| [Q07](#q07) | Corrispondenze teacher-surrogate | Esatta sui mapping dichiarati |
| [Q08](#q08) | Catena di evidenze e provenienza di una ipotesi | Esatta sui collegamenti |
| [Q09](#q09) | Ipotesi con evidenze contrastanti | Shortlist da disambiguare per dominio |
| [Q10](#q10) | Scelte che dipendono da assunzioni senza supporto registrato | Lacuna documentale |
| [Q11](#q11) | Affermazioni senza valutazioni delle evidenze | Lacuna documentale |
| [Q12](#q12) | Domini dichiarati contro domini delle evidenze | Audit di copertura, non prova di trasferimento |
| [Q13](#q13) | Origine documentata di una ipotesi | Provenienza, non certificazione |
| [Q14](#q14) | Decisioni collegate a ipotesi con controevidenza | Candidati alla revisione |
| [Q15](#q15) | Confronti allineati per specifica e blocco | Candidati con filtri conservativi |
| [Q16](#q16) | Differenza appaiata cambiando un fattore | Contrasto osservato, non causalità automatica |
| [Q17](#q17) | Interazione fattoriale 2×2 | Contrasto descrittivo per blocco completo |
| [Q18](#q18) | Curve di apprendimento a budget crescente | Esatta per osservazioni registrate |
| [Q19](#q19) | Test isolati e di ricomposizione di una sostituzione | Copertura esplicita e risultati da leggere |
| [Q20](#q20) | Predizioni preregistrate e misure corrispondenti | Affiancamento, non verdetto automatico |
| [Q21](#q21) | Riclassificazione degli esperimenti secondo oggetti diversi | Lente basata sulle ipotesi collegate |
| [Q22](#q22) | Ricerca trasversale per informazione, regime o failure mode | Ricerca lessicale, non classificazione automatica |
| [Q23](#q23) | Componenti senza esperimenti associati nel catalogo | Lacuna di collegamento |
| [Q24](#q24) | Componenti con test isolato associato ma nessuna ricomposizione | Shortlist, senza inferire successo |
| [Q25](#q25) | Protocolli senza controlli registrati | Audit del disegno |
| [Q26](#q26) | Copertura dei sottogruppi attesi nelle misure | Esatta solo per etichette concordate |
| [Q27](#q27) | Risultati positivi privi di una conferma collegata | Lacuna di conferma registrata |
| [Q28](#q28) | Ipotesi alternative ancora aperte e relative predizioni | Shortlist di spiegazioni non risolte |
| [Q29](#q29) | Esperimenti candidati allo stesso notebook | Compatibilità preliminare |
| [Q30](#q30) | Capacità con molte dipendenze a valle | Priorità strutturale candidata |
| [Q31](#q31) | Esperimenti che interrogano più ipotesi aperte | Shortlist informativa non probabilistica |
| [Q32](#q32) | Azioni pronte senza prerequisiti operativi incompleti | Filtro di workflow |
| [Q33](#q33) | Sincronizzazione e identità dei record | Controllo tecnico |
| [Q34](#q34) | Ambiguità che rendono inaffidabili le query | Controllo di qualità strutturale |

## Limiti dello schema emersi

1. **Componente effettivamente testato:** oggi si ricava indirettamente da sostituzioni e ipotesi; non esiste una relazione universale diretta esperimento → componente bersaglio.
2. **Successo di un test:** criteri/soglie sono testuali. Non esiste ancora una valutazione strutturata predizione → osservazione → verdetto con regola versionata.
3. **Regimi, informazione disponibile, cause del fallimento:** molte descrizioni sono testo; Q22 non le trasforma in tassonomie validate.
4. **Controlli obbligatori:** manca la relazione requisito di controllo → ipotesi → protocollo. Q25 rileva solo controlli registrati, non la loro sufficienza.
5. **Conferma riuscita e indipendente:** tipo/descrizione non sostituiscono un audit del risultato, dei seed e dei dati.
6. **Difficoltà × dimensione × budget:** i checkpoint danno la curva a budget crescente; ulteriori scaling richiedono valori numerici e unità coerenti, non parsing arbitrario del testo.
7. **Aggiornamento delle credenze:** le valutazioni sono storiche, ma non hanno ancora una catena di supersessione strutturata. Q09 non distingue automaticamente una controevidenza corrente da una valutazione poi ritirata.
8. **Parallelizzabilità:** stesso playground è un filtro iniziale. Dati condivisi, costo e vera indipendenza causale devono essere verificati prima dell'esecuzione.

Questi limiti sono proposte da discutere. Non sono stati aggiunti campi o tabelle in questa attività.

---

## Q01

### Scheda strutturale del componente

**Tipo:** Esatta sui collegamenti registrati.

**Parametri:** `component_id`.

Mostra una riga per oggetto. Non inventa stati o equazioni non registrati; fonti, evidenze e test si consultano con Q08/Q19/Q23.
...
### Decomposizione ricorsiva del modello

**Tipo:** Esatta, con limite di profondità.

**Parametri:** `model_id`, `component_id`, `max_depth`.

Conserva percorsi distinti se il grafo ha parti condivise. Evita cicli; un percorso registrato non prova una dipendenza causale.
...
### Ingressi, uscite e dipendenze dirette

**Tipo:** Esatta per il grafo dichiarato.

**Parametri:** `model_id`, `component_id`.

Legge accoppiamenti dichiarati, non inferisce dipendenze dalla sola correlazione tra variabili.
...
### Percorsi di informazione tra due componenti

**Tipo:** Topologica, non prova causale.

**Parametri:** `model_id`, `from_component`, `to_component`, `max_depth`.

Enumera percorsi semplici: non espande nel tempo le ricorrenze. Per il calcio, scegliere componenti collegati alle grandezze appropriate tramite porte/grandezze.
...
### Differenze tra componenti di due versioni

**Tipo:** Esatta solo con identificatori locali stabili.

**Parametri:** `model_a`, `model_b`.

Confronto testuale di metadati, non equivalenza matematica. ID mancanti/duplicati richiedono Q34. Q06 aggiunge l'inventario di equazioni e connessioni.
...
### Inventario matematico e connessioni per versione

**Tipo:** Inventario per confronto umano.

**Parametri:** `model_a`, `model_b`.

Non dichiara uguali equazioni solo perché i nomi coincidono. La corrispondenza cross-version delle porte va registrata o verificata.
...
### Corrispondenze teacher-surrogate

**Tipo:** Esatta sui mapping dichiarati.

**Parametri:** `teacher_model`.

Una corrispondenza registrata non equivale a una sostituzione validata.
...
### Catena di evidenze e provenienza di una ipotesi

**Tipo:** Esatta sui collegamenti.

**Parametri:** `claim_id`.

Esito esplicito, non voto dedotto dal nome della tabella. Il campo storico 'Risultati a sostegno' contiene anche risultati usati per contraddire: conta Esito.
...
### Ipotesi con evidenze contrastanti

**Tipo:** Shortlist da disambiguare per dominio.

**Parametri:** nessuno.

Conteggia valutazioni, non repliche indipendenti. Contrasti tra domini diversi non sono automaticamente contraddizioni.
...
### Scelte che dipendono da assunzioni senza supporto registrato

**Tipo:** Lacuna documentale.

**Parametri:** `model_id`.

Non dice 'falsa' né 'basata soltanto su assunzioni': altre giustificazioni possono esistere. Dice esattamente che manca supporto registrato per quella premessa.
...
### Affermazioni senza valutazioni delle evidenze

**Tipo:** Lacuna documentale.

**Parametri:** nessuno.

Assenza nel DB, non assenza nella letteratura.
...
### Domini dichiarati contro domini delle evidenze

**Tipo:** Audit di copertura, non prova di trasferimento.

**Parametri:** `claim_id`.

Identità dei record dominio, non inclusione semantica automatica tra condizioni testuali.
...
### Origine documentata di una ipotesi

**Tipo:** Provenienza, non certificazione.

**Parametri:** `claim_id`.

Una fonte di tipo Paper non è automaticamente una prova. La classificazione come derivazione o proposta si consulta anche in claims.Tipo ed evidence.
...
### Decisioni collegate a ipotesi con controevidenza

**Tipo:** Candidati alla revisione.

**Parametri:** nessuno.

Non chiama 'successiva' una valutazione senza date ISO affidabili. Non distingue automaticamente dipendenza necessaria da semplice associazione: è una shortlist.
...
### Confronti allineati per specifica e blocco

**Tipo:** Candidati con filtri conservativi.

**Parametri:** `evaluation_id`.

Stessa specifica, protocollo, blocco, seed, budget/unità, numerosità, sottogruppo e insieme di split. Richiede checkpoint con budget esplicito. Nessun controllo fallito ≠ tutti i controlli necessari superati. Hardware, stop rules e deviazioni vanno ancora esaminati.
...
### Differenza appaiata cambiando un fattore

**Tipo:** Contrasto osservato, non causalità automatica.

**Parametri:** `evaluation_id`, `factor_id`, `contrast_id`.

Non media run, checkpoint o osservazioni duplicate. Il segno è B−A: per una loss negativa è meglio, per F1 positiva è meglio. Variabili non registrate come fattori non risultano automaticamente controllate.
...
Calcola y11−y10−y01+y00; non p-value né significatività. Parametri livelli/fattori devono essere distinti e coerenti. Blocchi incompleti o celle duplicate non producono stime. Non è ancora una prova del meccanismo.
...
### Curve di apprendimento a budget crescente

**Tipo:** Esatta per osservazioni registrate.

**Parametri:** `evaluation_id`.

Una curva per run e sottogruppo, nessuna media tra checkpoint. I budget in unità diverse rimangono separati.
...
### Test isolati e di ricomposizione di una sostituzione

**Tipo:** Copertura esplicita e risultati da leggere.

**Parametri:** `substitution_id`.

Lo schema attuale non ha un booleano strutturato 'isolato superato/ricomposto fallito'. Questa query mostra modalità e risultati; l'esito richiede giudizio esplicito o una futura regola formalizzata.
...
### Predizioni preregistrate e misure corrispondenti

**Tipo:** Affiancamento, non verdetto automatico.

**Parametri:** `experiment_id`.

La soglia è testo: SQL non interpreta frasi come 'nettamente migliore'. Le date testuali non certificano una preregistrazione; il relativo artefatto/hash va verificato.
...
### Riclassificazione degli esperimenti secondo oggetti diversi

**Tipo:** Lente basata sulle ipotesi collegate.

**Parametri:** `axis`.

Una lente parametrica per componente, equazione, parametrizzazione, ricetta, meccanismo, modello o dominio. È pertinenza dichiarata, non prova che l'oggetto sia stato manipolato.
...
### Ricerca trasversale per informazione, regime o failure mode

**Tipo:** Ricerca lessicale, non classificazione automatica.

**Parametri:** `needle`.

Esempi di needle: 'calcio', 'storia temporale', 'plateau', 'ottimizzazione'. lower SQLite standard non offre case-folding Unicode completo. Una menzione non equivale a un'etichetta validata.
...
### Componenti senza esperimenti associati nel catalogo

**Tipo:** Lacuna di collegamento.

**Parametri:** `model_id`.

Non significa mai testati. Include associazioni dirette tramite sostituzione e indirette tramite ipotesi; manca ancora un contratto universale esperimento→componente testato.
...
### Componenti con test isolato associato ma nessuna ricomposizione

**Tipo:** Shortlist, senza inferire successo.

**Parametri:** `model_id`.

La presenza del protocollo non prova che sia stato eseguito né superato. Una query 'validato isolatamente ma non ricomposto' richiede anche una regola esplicita di successo.
...
### Protocolli senza controlli registrati

**Tipo:** Audit del disegno.

**Parametri:** `experiment_id`.

Non stabilisce quale controllo sia necessario e non considera un oracle equivalente a un controllo negativo. Per requisiti specifici serve registrare controllo richiesto→ipotesi.
...
### Copertura dei sottogruppi attesi nelle misure

**Tipo:** Esatta solo per etichette concordate.

**Parametri:** `evaluation_id`, `expected_strata_json`.

Esempio JSON ['riposo','subthreshold','spike','plateau'] con virgolette JSON doppie. Sono etichette da concordare, non dati già presenti. Conta misure, non episodi indipendenti.
...
### Risultati positivi privi di una conferma collegata

**Tipo:** Lacuna di conferma registrata.

**Parametri:** nessuno.

Un record di conferma presente non prova che la conferma sia completata o riuscita. Anche questo esito va formalizzato se vogliamo il filtro 'confermato con successo'.
...
### Ipotesi alternative ancora aperte e relative predizioni

**Tipo:** Shortlist di spiegazioni non risolte.

**Parametri:** nessuno.

Non dimostra compatibilità con tutti i dati. Servono predizioni strutturate e valutazioni di ciascun esito per eliminare automaticamente alternative.
...
### Esperimenti candidati allo stesso notebook

**Tipo:** Compatibilità preliminare.

**Parametri:** nessuno.

Stesso programma/playground non garantisce stessi dati, assenza di leakage o indipendenza. Confermare partizioni, generatori, budget e dipendenze; non autorizza esecuzione parallela automatica.
...
### Capacità con molte dipendenze a valle

**Tipo:** Priorità strutturale candidata.

**Parametri:** nessuno.

Conta dipendenze dichiarate dirette. Tenere distinto ipotizzato/verificato esaminando i record capdeps; non è utilità scientifica o necessità dimostrata.
...
### Esperimenti che interrogano più ipotesi aperte

**Tipo:** Shortlist informativa non probabilistica.

**Parametri:** nessuno.

Numero di ipotesi collegate ≠ numero di ipotesi distinguibili e ≠ guadagno informativo atteso. Servono contrasti discriminanti e, per una misura probabilistica, prior e likelihood.
...
### Azioni pronte senza prerequisiti operativi incompleti

**Tipo:** Filtro di workflow.

**Parametri:** nessuno.

Non interpreta i criteri testuali di avvio. 'Pronta' deve essere assegnato consapevolmente.
...
### Sincronizzazione e identità dei record

**Tipo:** Controllo tecnico.

**Parametri:** nessuno.

Zero righe indica nessuna intenzione pendente registrata, non garantisce che Airtable non sia stato modificato manualmente dopo l'ultimo import.
...
### Ambiguità che rendono inaffidabili le query

**Tipo:** Controllo di qualità strutturale.

**Parametri:** nessuno.

Le query che usano la CTE one escludono relazioni ambigue anziché scegliere arbitrariamente un estremo. Questa query le rende visibili; obbligatorietà e cicli richiedono ulteriori controlli.
