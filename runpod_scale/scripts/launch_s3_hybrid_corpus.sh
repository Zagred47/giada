...
S3_ROOT="${GIADA_S3_ROOT:-/workspace/giada-data/s3-hybrid-production-v1}"
BACKGROUND_ROOT="$S3_ROOT/background"
TARGETED_ROOT="$S3_ROOT/targeted"
COMPOSITE_ROOT="$S3_ROOT/composite"
...
run_component() {
  ...
  echo "[GIADA RunPod][S3] starting $label"
  ...
  while kill -0 "$launcher_pid" 2>/dev/null; do
    ...
    echo "[GIADA RunPod][S3][$label] shards $completed/$expected; failed markers $failed"
    sleep 30
  done
  if ! wait "$launcher_pid"; then
    echo "[GIADA RunPod][S3][$label] worker launcher failed; recent worker errors:" >&2
  ...
  if [[ "$completed" -ne "$expected" || "$failed" -ne 0 ]]; then
    echo "[GIADA RunPod][S3][$label] generation incomplete: completed=$completed/$expected failed=$failed" >&2
    for worker_log in "$output"/logs/worker-*.log; do
      [[ -f "$worker_log" ]] || continue
      echo "--- $(basename "$worker_log") ---" >&2
      tail -n 12 "$worker_log" >&2
  ...
  "$PYTHON_BIN" -m src.giada_runpod.cli validate \
    ...
    --output "$output"
  echo "[GIADA RunPod][S3] completed and validated $label"
}

plan_if_missing "$GIADA_ROOT/runpod_scale/configs/s3_hybrid_background.yml" "$BACKGROUND_ROOT"
plan_if_missing "$GIADA_ROOT/runpod_scale/configs/s3_hybrid_targeted.yml" "$TARGETED_ROOT"

run_component "long stochastic background" "$BACKGROUND_ROOT"
run_component "confirmed targeted episodes" "$TARGETED_ROOT"

"$PYTHON_BIN" -m src.giada_runpod.cli compose-s3 \
  --background "$BACKGROUND_ROOT" \
  --targeted "$TARGETED_ROOT" \
  --output "$COMPOSITE_ROOT"

echo "[GIADA RunPod][S3] production corpus complete: $COMPOSITE_ROOT"
