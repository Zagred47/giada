...
RESULT_ROOT="${GIADA_S3_RESULTS:-/workspace/giada-results/s3-matched-v1}"
CONFIG_PATH="$GIADA_ROOT/runpod_scale/configs/s3_matched_training.yml"
...
if [[ -f "$RESULT_ROOT/final_report.json" ]]; then
  echo "Completed S3 result already exists: $RESULT_ROOT/final_report.json" >&2
  exit 1
fi
...
echo "[GIADA RunPod][S3 matched] complete: $RESULT_ROOT/final_report.json"
