...
| Task 12: corrente analitica | Prima fase di `12_13_roadmap_current_and_timing.ipynb`. | **Conclusa nel playground Ca-HVA+pas**: ZIP verificato, path_full RMSE corrente 1.29e-6 mA/cm² contro 6.15e-5 di effect_full, con differenza guidata dal gate m. Nessuna prova sul neurone completo. |
| Task 13: allineamento temporale | Seconda fase dello stesso notebook, report distinto. | **Conclusa per il fixed-step Task7b**: `ica(t+dt)` usa V/m/h pre-step, max errore 1.30e-18 mA/cm²; conduttanza inferita, non registrata direttamente. |
| Task 14: variazione dei parametri | `14_roadmap_parameter_variation.ipynb`. | **Preregistrata, non ancora eseguita su Kaggle**: gbar, ECa, stati iniziali, maschera visibile/nascosta in matrice appaiata, senza riaddestramento. |
